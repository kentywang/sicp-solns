  ...
  (cond ((and (eq? target 'val) (not (eq? linkage 'return)))
         (make-instruction-sequence '(proc) all-regs
           `((assign continue (label ,linkage))
             (save continue) ; Evaluator application expects continue on stack.
             (goto (reg compapp)))))
...
(compile-and-go '(define (f x) (+ 5 (g x 100))))

;; If we type into evaluator (define (g x y) (* 2 x y)),
;; then (f 3) should be 605.