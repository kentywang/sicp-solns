...
;; 5.25 Strategy: Make easy, non-memoized version of lazy eval, so no thunks.
;; Steps:
;; 1. Eval operator.
;; 2. Check if primitive procedure or compound.
;; 3. If primitive, check if argument-less. If so, apply immediately, else eval args.
;; 4. If compound, apply immediately with unevaluated operands as the args.
  ...
  (make-machine
    ...
    '((assign continue (label done))
      eval-dispatch
        ...
        ;(goto (label unknown-expression-type)) ; No handling for now
        (goto (label done))
      ev-self-eval
        (assign val (reg exp))
        (goto (reg continue))
      ...
      ev-appl-did-operator
        ...
        (branch (label primitive-proc-operand-check))
      ...
      primitive-apply
        ...
        (goto (reg continue))
      compound-apply
        (assign argl (reg unev)) ; Unevaluated operands are in unev, move to argl.
        (assign unev 
