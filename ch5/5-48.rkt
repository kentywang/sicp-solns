...
;; Not storing compile-and-run as a primitive procedure since that flow just puts the
;; instructions into the val register and then returns. I think we need compile-and-run
;; to be a primitive operation for the machine. Whether it belongs in the
;; apply-dispatch branch or as a special form is subjective, I guess.
...
(define (compile-and-run-exp expression)
  (text-of-quotation (cadr expression)))
  ...
  (make-machine
    ...
    '(
      ...
      compile-branch
        ...
        ;; Continue is already in register and not on stack, so don't need save/restore.
        (goto (reg val))
      ev-self-eval
        (assign val (reg exp))
        (goto (reg continue))
...
;; Since first instruction is a branch, it expects the flag register to be either
;; true or false. Since it's *unassigned*, it is truthy. Thus we need to set flag
;; register to false at the start, or we could add some test before the first branch
;; to get it set indirectly.
(set-register-contents! 
  eceval 'flag false)
;(eceval 'trace-on)
