...
(define (merge-frames f1 f2)
  (define (compat? a b)
    (if (or (null? a)
            (null? b))
        true
        (let* ((a-binding (mcar a))
               (a-var (binding-variable a-binding))
               (b-binding (binding-in-frame a-var b)))
          (cond ((or (equal? a-binding b-binding)
                     (false? b-binding))
                 (compat? (mcdr a) b))
                (else false)))))
  (if (compat? f1 f2)
      (mappend f1 f2) ; We'll have dupes but it will still work.
      false))

;; 4.76
(define (conjoin-2 conjuncts frame-stream)
  (define (get-merged-frames s1 s2) ; Returns stream of compatible frames between s1, s2.
  ...
  (if (null? conjuncts)
      the-empty-stream
      (let ((merged-frames (get-merged-frames (qeval (first-conjunct conjuncts)
                                                     frame-stream)
                                              (conjoin-2 (rest-conjuncts conjuncts)
                                                         frame-stream))))
        ;(display "Merged frames:")(newline)(display-stream merged-frames)
        (stream-filter true? merged-frames))))

;; 4.76
(define (stream-memq find s)
  (if (stream-null? s)
      false
      (if (eq? (stream-car s) find)
          s
          (stream-memq find (stream-cdr s)))))
...
(define (extend variable value frame)
  (mcons (make-binding variable value) frame))
...
(run-query '(und (job ?person (computer programmer))
                 (address ?person ?where)))

(run-query '(and (supervisor ?x (Bitdiddle Ben))
                 (not (job ?x (computer programmer)))))
;; The "parallelized" nature of this and means that not clauses don't work
;; since we start with an empty frame to filter.
(run-query '(und (supervisor ?x (Bitdiddle Ben))
                 (not (job ?x (computer programmer)))))

(run-query '(and (supervisor 
                  ?middle-manager ?person)
                 (supervisor
                  ?x ?middle-manager)
                 (job ?x (computer programmer))))
(run-query '(und (supervisor 
                  ?middle-manager ?person)
                 (supervisor
                  ?x ?middle-manager)
                 (job ?x (computer programmer))))