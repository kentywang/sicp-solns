...
(define scheme-machine
  (make-machine
    ...
    '(read-eval-print-loop
      ...
      ev-application
        (save continue)
        (assign unev (op operands) (reg exp)) ; We can also not save unev too, if symbol.
        (assign exp (op operator) (reg exp))
        (test (op symbol?) (reg exp)) ; Could be lambda too.
        (branch (label ev-appl-sym-operator))
        (save env)
        (save unev)
        (assign
         continue (label ev-appl-did-operator))
        (goto (label eval-dispatch))
      ev-appl-sym-operator
        (assign
         continue (label ev-appl-did-sym-operator))
...
;;; 2.
;
;Don't think so. For instance, if you were to run the same expression over and
;over in an evaluator, each time would still require a eval-dispatch case
;analysis. With compiliation, you would have just the instructions to perform
;the action, something that can't happen in an interpreter, unless there was
;some magical caching involved. So peephole optimizations are only a small
;portion of the advantages offered by compiliation.