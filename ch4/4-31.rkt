...
;; Strategy: internal representation for procedure remains the same (though it keeps
;; the eval type within the parameters element). Modify parameter selector to iterate
;; through list elements properly. Add new selector for getting eval type for each
;; parameter. This approach is simpler for representing procedures but involves more
;; duplicated work when repeatedly applying a procedure.

;; For memoization option, simply create a new type for unmemoized thunks to add to
;; case analyses for force-it and list-of-delayed-args (renamed to list-of-delayable-
;; args).
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           ...
           (list-of-delayable-args 
            arguments 
            env
            (procedure-parameter-eval-types procedure)) ; 4.31
...
(define (thunk-unmemo? obj) (tagged-list? obj 'thunk-unmemo))
...
;; 4.31:
(define (list-of-delayable-args exps env parameter-eval-types)
  (if (no-operands? exps)
      '()
      (let ((eval-type (car parameter-eval-types)))
        (display parameter-eval-types)
        (display " ")
        (display (first-operand exps))
        (newline)
        (cons ((cond ((eq? eval-type 'applicative) actual-value)
                     ((eq? eval-type 'lazy-memo) delay-it)
                     ((eq? eval-type 'lazy) delay-it-unmemo)
                     (else (error "Unsupported eval-type" eval-type)))
               (first-operand exps) 
               env)
              (list-of-delayable-args 
               (rest-operands exps)
               env
               (cdr parameter-eval-types))))))
...
;; 4.31 new addition:
(define (procedure-parameter-eval-types p)
  (map (lambda (x)
         ; (display x)
         ; (newline)
         (if (symbol? x)
...
; (define (try a (b lazy) c (d lazy-memo)) (if (= a 0) 1 b))
; (try (/ 1 0) 2 3 4) ; Should error
; (try 0 (/ 2 0) 3 (/ 4 0)) ; 1