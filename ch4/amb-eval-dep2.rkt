...
(provide (all-defined-out)  ;; :TODO define explicitly which procedures are provided
         (all-from-out "amb-eval-dep3.rkt")
         (all-from-out "amb-eval-dep4.rkt"))
...
(define (person<? person1 person2)
  (string<? (person->string person1)
            (person->string person2)))

(define (person->string p)
  (apply-in-underlying-scheme string-append
                              (map symbol->string p)))
...
;
;; We use a special printing procedure, user-print, to avoid printing the environment part of a compound procedure, 
